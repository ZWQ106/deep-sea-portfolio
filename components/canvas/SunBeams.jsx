...
const BeamMaterial = shaderMaterial(
  { uColor: new THREE.Color("white"), uOpacity: 0.5, uTime: 0 },
...
export default function SunBeams({ count = 12 }) {
  const group = useRef();
  const beams = useMemo(() => new Array(count).fill().map(() => ({
      x: (Math.random() - 0.5) * 60, z: (Math.random() - 0.5) * 20 - 5,
      scaleX: Math.random() < 0.4 ? 5 + Math.random() * 5 : 20 + Math.random() * 15,
      scaleY: Math.random() < 0.4 ? 60 + Math.random() * 30 : 40 + Math.random() * 20,
      opacity: Math.random() < 0.4 ? 0.04 + Math.random() * 0.04 : 0.015 + Math.random() * 0.02,
      speed: Math.random() < 0.4 ? 0.5 : 0.2, rotation: (Math.random() - 0.5) * 0.3
  })), [count]);

  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (group.current) group.current.children.forEach((mesh, i) => { mesh.rotation.z = beams[i].rotation + Math.sin(t * beams[i].speed + i) * 0.02; });
  });
